...
classdef BEAM
    ...
    methods
        ...
        function obj = BEAM(NAME, NODES, ELEMENTS, fdofs_restrained, fdofs_free)
        ...
        function obj = assemble (obj,unitW,unitW_tie,load_additional,dead,dofs_restrained)
            ...
            obj.SW = zeros(3*nodes,1); % Initialize a vector w/ the length of the no. of dof
            ...
            for E = 1:elements % loop through all elements & build stiffness matrix
                ...
                dof11 = obj.NODES.dofs(n1,1); dof12 = obj.NODES.dofs(n1,2); dof13 = obj.NODES.dofs(n1,3); % element node 1 - dofs
                dof21 = obj.NODES.dofs(n2,1); dof22 = obj.NODES.dofs(n2,2); dof23 = obj.NODES.dofs(n2,3); % element node 2 - dofs
                ...
                ka = obj.ELEMENTS.EAEI(E,1)/Le; % generic stretching stiffness term
                kb = obj.ELEMENTS.EAEI(E,2)/Le^3; % generic bending stiffness term
                g = 9.81; % m/s2 - gravity
                ...
                % Self Weight: element contribution to node
                if E >= 1 && E <= elements - 1
                    obj.SW(dof12) = obj.SW(dof12) + 0.5*Le*unitW*g; % N - SW contribution to first node
                    obj.SW(dof22) = obj.SW(dof22) + 0.5*Le*unitW*g; % N - SW contribution to second node
                else
                    obj.SW(dof12) = obj.SW(dof12) + 0.5*Le*unitW_tie*g; % N - SW contribution to first node
                    obj.SW(dof22) = obj.SW(dof22) + 0.5*Le*unitW_tie*g; % N - SW contribution to second node
                end
                
                %Updating the global applied nodal force vector {fa} coefficients
                
                obj.fa = -dead*obj.SW - load_additional; % Multiply SW by partial load factor
        ...
        function obj = length (obj)
            ...
            for E = 1:elements % loop through all elements & build stiffness matrix
                ...
                n1 = obj.ELEMENTS.nodes(E,1); n2 = obj.ELEMENTS.nodes(E,2); % identify element node numbers
                x1 = obj.NODES.coords(n1,1); y1 = obj.NODES.coords(n1,2); % element node 1 - x,y coordinates
                x2 = obj.NODES.coords(n2,1); y2 = obj.NODES.coords(n2,2); % element node 2 - x,y coordinates c = cos(alpha); c2 = c*c; s = sin(alpha); s2 = s*s; cs = c*s; % angle parameters
                Le = sqrt( (x2 - x1)^2 + (y2 - y1)^2 ); % element length
        ...
        function obj = plotting (obj,amp)
            ...
            for E = 1:elements
                ...
                ux1 = obj.D(obj.NODES.dofs(n1,1)); uy1 = obj.D(obj.NODES.dofs(n1,2)); thz1 = obj.D(obj.NODES.dofs(n1,3)); % element node 1 - global dofs
                ux2 = obj.D(obj.NODES.dofs(n2,1)); uy2 = obj.D(obj.NODES.dofs(n2,2)); thz2 = obj.D(obj.NODES.dofs(n2,3)); % element node 2 - global dofs
        ...
        %% %%%%% PART SIX A: OBTAIN THE MAXIMUM VERTICAL DISPLACEMENT %%%% %%
        
        function obj = FindMaxVertDisp (obj)
            
            obj.maxvertdisp = min (obj.D(2:3:end-1));
            
        end

                %% %%%%% PART SIX B: OBTAIN THE MAXIMUM LATERAL DISPLACEMENT %%%% %%
        ...
        function obj = axial (obj)
            ...
            for E = 1:elements
                ...
                % Obtain axial force
                obj.Faxial(E) = obj.ELEMENTS.EAEI(E,1)*eps ; 
                
            end
        end

        %% %%%%%%%%%  PART EIGHT: OBTAIN THE SELF WEIGHT %%%%%%%%%% %%
        
        function obj = SelfWeight (obj,unitW,load_additional)
            ...
            % Output total SW of structure
            obj.combined = obj.SW + load_additional; 
            obj.selfweight = sum(obj.combined); % Essential BC
            
        end

         %% %%%%%%%%%%  PART Nine: Plot Elements that fail  %%%%%%%%%% %%
        ...
        function obj = buckleplot (obj,memberfail)
         ...
         %% %%%%%%%%%%  PART Ten: Plot Elements that yield  %%%%%%%%%% %%
        
        function obj = yieldplot (obj,memberfail)
            ...
            xlabel('x coordinate [mm]','FontSize',16);
            ylabel('y coordinate [mm]','FontSize',16);
            title(['Yielding Members'],'FontSize',16);
