...
obj2 = obj2.buckleplot(memberfail_canbuck);
obj2 = obj2.yieldplot(memberfail_canyield);
                    
mass = (d+sqrt(2)*d + sqrt((span_arch/m)^2+d^2)) * unitW_arch *(m+1) + sum(obj1.Length) * unitW_can* (m+1) + sum(obj2.Length) * unitW_arch + load_longtruss*(n+length(restrainlong))/g;
