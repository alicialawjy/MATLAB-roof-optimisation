...
radius = 90000:500:span_arch; % minimum radius cannot be less than half of the span
        ...
        for j = 0: length(longnodes) % eg: from (n-1) nodes, choose (j) nodes to restrain
            ...
            for k = 1:size(combi,1) % the particular combination of bottom chord nodes to restrain
                ...
                if fail == 0
                    ...
                    minFOS_overall = min ([minFOS_canyield, minFOS_canbuck, minFOS_archyield,minFOS_archbuck, FOS_tie]);
                    mass = ...
                        (d+sqrt(2)*d + sqrt((span_arch/m)^2+d^2)) * unitW_arch *(m+1) + ...
                        sum(obj1.Length) * unitW_can* (m+1) + ...
                        sum(obj2.Length(1:end-1)) * unitW_arch + ...
                        load_longtruss*(n+length(longbrace{count}))/g + ...
                   ...
                   % store configuration
                    pass = [pass; radius(rloop), depth(dloop), m, n, minFOS_overall,tonage, obj2.maxlatdisp, vertreaction, forceintie];
                    FOSpass = [FOSpass; minFOS_canyield, minFOS_canbuck, minFOS_archyield ,minFOS_archbuck, FOS_tie];
...
fprintf('Force in Tie = %g N\n', pass(index,9))
longbrace_formin = longbrace {index};

figure
plot(pass(:,2),pass(:,6),'x')
title ('Depth vs Tonnage')
xlabel ('Depth [mm]')
ylabel ('Tonnage [kg/m2]')
hold off
