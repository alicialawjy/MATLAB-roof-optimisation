% Roof Optimisation - Cantilever and Main Arch combined PIN PIN
...
%% Loop 1: [] - Loop through rise by changing centerline radius
radius = 75000:500:span_arch; % minimum radius cannot be less than half of the span

for rloop = 1:length(radius)
    
    ri = radius(rloop);

    %% Loop 2: [] - Loop through depth
    depth = 5*1000:500:10*1000;
    for  dloop = 1:length(depth)  % Loop through depth of arch and cantilever
        ...
        dofsarch_free = [1:3*(m+1),3*(m+2):3*(nodes_arch-1),3*nodes_arch];
        dofsarch_restrained = [3*(m+2)-2,3*(m+2)-1,3*nodes_arch-2, 3*nodes_arch-1]; 
        ...
        load_additionalcan (3*(n+2)-1) = load_longtruss; % first node definitely restrained
        load_additionalcan (3*(nodes_can)-1) = (width_can + sqrt(width_can^2 + d^2)) * unitW_can * g; % last node definitely restrained
        longnodes = n+3: nodes_can-1; % possible options of intermediate bottom chord nodes to restrain

        for j = 0: length(longnodes) % eg: from (n-1) nodes, choose (j) nodes to restrain
            
            combi = nchoosek(longnodes,j); % gives all possible combinations
            
            for k = 1:size(combi,1) % the particular combination of bottom chord nodes to restrain
                
                c = combi (k,:); % Current restraint combination
                ...
                % Update longitudinal brace loadings
                if c == 1 % when choose 0 nodes to restrain
                    c = [];
                else
                    for l = 1:size(combi,2) % add loads to nodes with longitudinal restrain
                        
                        load_additionalcan (3*c(l)-1) = load_additionalcan (3*c(l)-1) + load_longtruss;
                ...
                if obj1.maxvertdisp > span_can/180 % failed deflection
                    ...
                    fail = 1;
                    break %failed --> out of config (NEXT!)
                ...
                for p = 1:elements_can
                    ...
                    if FOS_caninbuck < 1 
                        ...
                        fail = 1;
                        break %  Failed --> Out of the buckling check for loop
                        
                    % Sieve #3: Yielding
                    elseif FOS_canyield < 1
                        
                        fail = 1;
                        break %  Failed --> Out of the yield check for loop
                ...
                if fail == 1
                    break % Out of check --> out of configuration (NEXT!)
                else
                    
                end
                                    
                % ----------- CHECK #4a: Out of Plane Buckling --------- %
                restrained = [n+2; c.'; nodes_can]; % nodes restrained out of plane
                distance = diff(restrained); % number of members between each restrain
                ...
                for q = 1:length(distance) % Checking between longitudinal restrains
                    ...
                    if fail == 1
                        break % out of current restrain loop --> out of buckling check
                ...
                if fail == 1
                    break % out of buckling check --> out of configuration (NEXT!)
                ...
                obj2 = obj2.assemble (unitW_arch,unitW_arch, load_additionalarch,dead,dofsarch_restrained);
                obj2 = obj2.solve ();
                obj2 = obj2.newcoord (amp);
                obj2 = obj2.FindMaxVertDisp ();
                obj2 = obj2.axial ();
                obj2 = obj2.length ();
                obj2 = obj2.SelfWeight (unitW_arch,load_additionalarch);
                ...
                % -------- CHECK #3b+#4b: Yielding + InBuckling --------- %
                for p = 1:elements_arch
                    ...
                    % In Plane Buckling using Perry-Robertson a curve
                    if FOS_archinbuck < 1 % Only interested with members in compression
                        
                        fail = 1;
                        break % failed --> out of buckling check
                        
                        % Check #3: Yielding
                    elseif FOS_archyield < 1
                        
                        fail = 1;
                        break % failed --> out of yielding check
                ...
                if fail == 0
                    count = count+1;
                    longbrace {count} = [n+2, c, 2*n+2];
                    minFOS_overall = min([minFOS_canyield minFOS_canbuck minFOS_archyield minFOS_archbuck]);
                    mass = (d+sqrt(2)*d + sqrt((span_arch/m)^2+d^2)) * unitW_arch *(m+1) + sum(obj1.Length) * unitW_can* (m+1) + sum(obj2.Length) * unitW_arch + load_longtruss*(n+length(longbrace{count}))/g;
                    tonage = mass*1000000/(span_arch*length_roof/2);
                    horireactionL = obj2.F(3*(m+2)-2);
                    vertreactionL = obj2.F(3*(m+2)-1);
                    
                    % store configuration
                    pass = [pass; radius(rloop), depth(dloop), m, n, minFOS_overall,tonage, horireactionL, vertreactionL];
                    FOSpass = [FOSpass; minFOS_canyield, minFOS_canbuck, minFOS_archyield ,minFOS_archbuck];
                else
                end
            end
        end
...
index = find(pass(:,6) == min(pass(:,6)),1);
fprintf('For a minimum tonage of %g kg/m2\n', pass(index,6))
fprintf('Radius = %g mm,\n', pass(index,1))
fprintf('Depth = %g mm,\n', pass(index,2))
fprintf('Min FOS = %g\n', pass(index,5))
fprintf('Horizontal Reaction = %g mm\n',pass(index,7))
fprintf('Vertical Reaction = %g N\n', pass(index,8))
longbrace_formin = longbrace {index};

figure
plot(pass(:,2),pass(:,6),'x')
title ('Tonnage')
hold off         