...
% USE: Members Bars - 550 x 25 CHS
fy_arch = 275; % N/mm - steel grade
A_arch = 419*100; % mm2 - cross-sectional area of each member
I_arch = 149822*10^(4); % mm4 - second moment of area
EA_arch = E*A_arch; % N - axial rigidity of all members
EI_arch = E*I_arch; % Nmm2 - flexural rigidity of all members
unitW_arch = 329/1000; % kg/mm2 - mass per meter of CHS
        ...
        for j = 0: length(longnodes) % eg: from (n-1) nodes, choose (j) nodes to restrain
            ...
            for k = 1:size(combi,1) % the particular combination of bottom chord nodes to restrain
                ...
                if fail == 0
                    ...
                    % store configuration
                    pass = [pass; radius(rloop), depth(dloop), m, n, minFOS_overall,tonage, obj2.maxlatdisp, vertreaction];
...
index = find(pass(:,6) == min(pass(:,6)));

fprintf('For a minimum tonage of %g kg/m2\n', pass(index,6))
fprintf('Radius = %g mm,\n', pass(index,1))
fprintf('Depth = %g mm,\n', pass(index,2))
fprintf('Min FOS = %g\n', pass(index,5))
fprintf('Lateral Displacement = %g mm\n',pass(index,7))
